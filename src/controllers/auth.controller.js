...
const logoutUser = asyncHandler(async (req, res) => {
  ...
  // if deviceType is web then no need to check deviceToken
  if (deviceType !== 'web' && !deviceToken) {
    throw new ApiError(400, "Device token is required for mobile devices");
  }
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  try {
    ...
    if (!incomingRefreshToken) {
      throw new ApiError(404, "Invalid Token Or Token Missing");
    }
    ...
    throw new ApiError(404, error?.message || "Invalid Token Or Token Missing");
  }
});
